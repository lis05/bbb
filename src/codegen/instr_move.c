...
#define THRESHOLD 128
    gpr_reg_t temp1 = NULL, temp2 = NULL;
    if (len >= THRESHOLD) {
        const char *loop_label = lblg_gen(lblg);
        const char *end_label = lblg_gen(lblg);
        temp1 = gpr_pool_please(pool, frag);
        temp2 = gpr_pool_please(pool, frag);
        ...
        cb_add_back(cb, indent + CB_TAB, "mov %s, qword [%s + %s]\n", temp2->qname,
                    util_format_memory_address2(from, -8), temp1->qname);
        cb_add_back(cb, indent + CB_TAB, "mov qword [%s + %s], %s\n",
                    util_format_memory_address2(to, -8), temp1->qname, temp2->qname);
    ...
    for (size_t i = 0; i + 8 <= len; i += 8) {
        ...
        cb_add_back(cb, indent, "mov qword [%s], %s\n",
                    util_format_memory_address2(to, i), temp2->qname);
    }
    ...
    if (len >= 4) {
        ...
        cb_add_back(cb, indent, "mov dword [%s], %s\n",
                    util_format_memory_address(to), temp2->dname);
        from.offset += 4;
        to.offset += 4;
        len -= 4;
    }
    ...
    if (len >= 2) {
        ...
        cb_add_back(cb, indent, "mov word [%s], %s\n",
                    util_format_memory_address(to), temp2->wname);
        from.offset += 2;
        to.offset += 2;
        len -= 2;
    }
    ...
    if (len >= 1) {
        ...
        cb_add_back(cb, indent, "mov byte [%s], %s\n",
                    util_format_memory_address(to), temp2->bname);
        len -= 1;
    }
